...
def main():
    ...
    if training_args.do_train:
        all_column_names = list(raw_datasets["train"].features)
    else:
        all_column_names = list(raw_datasets["validation"].features)
    column_names = all_column_names[0:2]
    ...
    def generate_and_tokenize_prompt(data_point):
        if (len(data_point[input_column_name]) > data_args.max_source_length):
            data_point[input_column_name] = data_point[input_column_name][:data_args.max_source_length]
        if (len(data_point[target_column_name]) > data_args.max_target_length):
            data_point[target_column_name] = data_point[target_column_name][:data_args.max_target_length]
        
        messages = [{"role": "system", "content": ""}]
        messages.append(
                {"role": "user", "content": data_point[input_column_name]}
            )
        ...
        target_text = data_point[target_column_name] + "<|eot_id|>"
    ...
    with training_args.main_process_first(desc="dataset map tokenization"):
        if not data_args.streaming:
            tokenized_datasets = raw_datasets.map(
                ...
                remove_columns=all_column_names,
    ...
    if training_args.do_eval:
        ...
        metric = evaluate.load("./src/ft_llama3_lora/accuracy.py")
